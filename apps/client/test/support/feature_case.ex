    ...
    {:ok, session} =
      Wallaby.start_session(
        ...
        create_session_fn: fn url, capabilities ->
          ...
          # don't get a GPU, but WebGL stuff (Three.js) needs one.
          # --use-gl=angle selects ANGLE as the GL backend; --use-angle=swiftshader
          # tells ANGLE to use SwiftShader (software rasterizer) so WebGL works
          # in headless CI without a real GPU.
          # https://stackoverflow.com/a/73048626
          args = ["--use-gl=angle" | args]
          args = ["--use-angle=swiftshader" | args]
